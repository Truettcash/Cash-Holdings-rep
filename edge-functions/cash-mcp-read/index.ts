...
async function fetchMany<T extends Record<string, unknown>>(
  ...
  if (error) throw Object.assign(new Error("QUERY_FAILED"), { code: "QUERY_FAILED" as const, detail: error.message });
  return (data ?? []) as T[];
}
  ...
  switch (payload.action) {
    case "list_brands": {
      const rows = await fetchMany<BrandRow>(
        client
          .from("brands")
          .select("id,key,name,slug,status,tagline,description,accent_color,created_at")
...
export default async function handler(req: Request) {
  return await handleCashMcpRead(req);
}
