...
import { searchWhatCashKnows } from "@/lib/jarvis/search";
...
export function CommandPalette() {
  ...
  const { setOpen: setJarvisOpen, ask } = useJarvis();
  ...
  // KNOWLEDGE resolves through the governed read layer, same as Open Knowledge.
  const known = useQuery({
    queryKey: ["known", "palette", term],
    queryFn: () => searchWhatCashKnows(term, ["knowledge", "intelligence"]),
    enabled: paletteOpen && term.trim().length > 2,
    staleTime: 30_000,
    retry: false,
  });
  ...
  return (
    <CommandDialog open={paletteOpen} onOpenChange={setPaletteOpen}>
      <CommandInput
        ...
        placeholder="Search knowledge, work, CRM, intelligence… or run a command"
      />
      <CommandList>
        ...
        <CommandGroup heading="Commands">
          <CommandItem
            value="jarvis ask operator assistant"
            onSelect={() => {
              setPaletteOpen(false);
              setJarvisOpen(true);
              if (term.trim().length > 2) void ask(term.trim());
            }}
          >
            <span className="font-sans text-[10px] uppercase tracking-wider text-teal mr-2">JVS</span>
            {term.trim().length > 2 ? `Ask Jarvis: “${term.trim()}”` : "Open Jarvis"}
          </CommandItem>
        ...
        <CommandGroup heading="Knowledge">
          {(known.data ?? []).slice(0, 20).map((r) => (
            <CommandItem
              key={`${r.category}-${r.id}`}
              value={`knowledge ${r.title} ${r.type} ${r.context ?? ""}`}
              onSelect={() => go(r.to ?? "/knowledge")}
            >
              <Tag>{r.category === "intelligence" ? "MEM" : "KNW"}</Tag>
              <span className="truncate">{r.title}</span>
              <span className="ml-auto font-sans text-[10px] text-muted-foreground">{r.type}</span>
            </CommandItem>
          ))}
        </CommandGroup>
