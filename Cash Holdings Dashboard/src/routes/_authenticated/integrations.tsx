...
import { Instagram, RefreshCw, Link2, Unlink, AlertTriangle, ShoppingBag, Youtube } from "lucide-react";
...
import { startYouTubeConnect } from "@/lib/integrations/youtube";
import { integrationAccountsQuery, integrationSyncRunsQuery } from "@/lib/integrations/queries";
import { connectorHealth } from "@/lib/integrations/connector";
...
import { McpTokenCopy } from "@/components/integrations/mcp-token-copy";
import { JarvisConnectCard } from "@/components/integrations/jarvis-connect";
...
const UNWIRED_NOTICE: Record<string, string> = {
  ...
  "google-analytics": "Google Analytics isn't wired to the connector service yet.",
};
...
function IntegrationsPage() {
  ...
  const [expanded, setExpanded] = useState<"instagram" | "ebay" | "youtube" | null>(null);
  ...
  const youtubeChannels = useMemo(
    () =>
      (channels.data ?? []).filter((c) => (c.channel_type ?? "").toLowerCase() === "youtube"),
    [channels.data]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <div className="mono-label !text-[9px]">LOCAL RUNTIME</div>
            ...
            {CONNECTED_CATALOG.map((entry) => {
                  ...
                  <IntegrationCard
                    ...
                    accountLabel={
                      youtubeSummary.connected
                        ? `${youtubeSummary.count} channel${youtubeSummary.count === 1 ? "" : "s"} connected`
                        : undefined
                    }
                    lastSyncedAt={youtubeSummary.lastSyncedAt}
                    health={youtubeSummary.lastError ? "error" : "ok"}
              ...
              return (
                <IntegrationCard
                  ...
                  onConnect={() =>
                    toast.message(
                      UNWIRED_NOTICE[entry.id] ??
                        `${entry.name} isn't wired to the connector service yet.`
                    )
                  }
                />
              );
      ...
      {expanded === "youtube" && (
        <section className="glass-panel rounded-lg divide-y divide-hairline ch-fade-in">
          <div className="px-3 py-2 flex items-center gap-2">
            <Youtube className="h-3.5 w-3.5 text-teal" />
            <div className="mono-label !text-[9px]">YOUTUBE · PER CHANNEL</div>
            <div className="ml-auto mono-label !text-[9px] text-foreground/50">
              {youtubeChannels.length} CHANNELS
            </div>
          </div>
          {youtubeChannels.map((c) => (
            <YouTubeChannelRow key={c.id} channelId={c.id} name={c.name} handle={c.handle} />
          ))}
          {channels.isLoading && <SkeletonRows rows={3} className="p-3" />}
          {!channels.isLoading && youtubeChannels.length === 0 && (
            <EmptyState
              title="NO YOUTUBE CHANNELS"
              hint="Add a channel with type youtube before connecting."
            />
          )}
        </section>
      )}
...
/** One YouTube channel row: starts the provider-specific OAuth flow with the
 *  real `channels.id`. Status comes from the page's authenticated account read;
 *  sync/refresh/disconnect are not offered because production has no endpoint. */
function YouTubeChannelRow({
  ...
  const connect = async () => {
    setBusy(true);
    try {
      const url = await startYouTubeConnect(channelId);
      window.location.assign(url);
    } catch (err: any) {
      toast.error(err?.message ?? "YouTube connect failed");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="px-3 py-2 flex items-center gap-3 text-[12px]">
      <span className="truncate">{name}</span>
      {handle && (
        <span className="mono-label !text-[9px] text-muted-foreground truncate">{handle}</span>
      )}
      <button
        onClick={connect}
        disabled={busy}
        className="ml-auto inline-flex items-center gap-1.5 rounded-md border border-edge px-2 py-1 text-[11px] hover:bg-[var(--surface-2)] disabled:opacity-50"
      >
