-- ATHRTY Stripe reconciliation deployment candidate v1.
-- PREPARED ONLY. Do not apply to production without explicit operator approval.
-- Source parity target: supabase/schema/athrty_process_stripe_event_v1.sql on main.
-- Includes three reviewed hardening changes:
--   1) partial unique-index conflict inference
--   2) removal of invalid PaymentIntent bigint -> timestamptz assignments
--   3) cumulative-safe refund/dispute reconciliation
