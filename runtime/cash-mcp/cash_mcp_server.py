"""OpenJarvis-compatible JSON-RPC stdio Cash MCP server."""
...
class CashMcpServer:
    def __init__(self, gateway: GatewayClient, token_provider: TokenProvider, *, stderr: TextIO | None = None) -> None:
        ...
        try:
            if method == "initialize":
                return _response(request_id, {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "cash-mcp", "version": "1.0.0"},
                })
            if method == "tools/list":
                return _response(request_id, {"tools": [
                    {"name": name, "description": TOOL_DESCRIPTIONS[name], "inputSchema": TOOL_SCHEMAS[name]}
            ...
            return _response(request_id, error={"code": INTERNAL_ERROR, "message": str(exc)})
        except GatewayError as exc:
            return _response(request_id, {"content": [{"type": "text", "text": json.dumps(exc.payload, sort_keys=True)}], "isError": True})
        except Exception as exc:  # pragma: no cover - defensive protocol boundary
            self.stderr.write(f"cash-mcp internal error: {type(exc).__name__}\n")
...
def serve_lines(server: CashMcpServer, stdin: TextIO = sys.stdin, stdout: TextIO = sys.stdout) -> None:
    for line in stdin:
        try:
...
def main() -> None:  # pragma: no cover - process wiring
    raise SystemExit("Configure GatewayClient and TokenProvider in bootstrap before running cash-mcp")
